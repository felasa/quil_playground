...
(defn circle-field
  "Return vector tangent to circle with radius at [x y], centered at middle"
  [W H [x y]]
  (let [angle (- (math/atan2 (- (/ W 2) x) (- (/ H 2) y)))]
...
(q/defsketch circledrops
  ...
  :settings #(q/smooth)
  :setup setup
  :draw #(iterate-drawing-circle 25)
  :size [800 800]
  :features [:resizable]
  :renderer :java2d)

(comment 
  (quil.applet/with-applet sketch.circledrops/example 
