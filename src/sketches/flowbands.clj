...
(defn draw-bands
  ...
  (let [hsb-vals (shuffle (get pallete :hsb))
    ...
    (dotimes [_ n-bands]
      (let [x (rand-int (q/width)) y (rand-int (q/width))
        ...
        (let [texture (rand-nth [:circle :line :none])]
          ;why are textures going off the band?
          (case texture
            :circle
            (do
              (q/no-fill)
   ...
   #_(doseq [x (range 100 700) y (range 200 600)]
...
(q/defsketch simple-bands
  ...
  :settings #(q/smooth)
  :draw #(draw-bands 10 pallete)
  :size [800 800])

(comment
  (quil.applet/with-applet sketches.flowbands/simple-bands
